...
namespace ConsoleApp1.Controllers
{
    ...
    public class MemberController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetAllCustomers()
        {
            try
            {
                _logger.LogInformation("获取所有客户列表");
                
                // 调用MemberService中的私有方法不可行，我们直接查询数据库
                var result = await _memberService.GetDebugCustomerListAsync();
        ...
        [HttpGet("debug/customer/{customerId}/orders")]
        public async Task<ActionResult> GetCustomerOrders(int customerId)
        {
            try
            {
                _logger.LogInformation($"获取客户 {customerId} 的订单数据");
                
                var orders = await _memberService.GetDebugCustomerOrdersAsync(customerId);
                
                return Ok(new { 
                    message = $"客户 {customerId} 订单数据获取成功",
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new { message = "获取订单数据失败", error = ex.Message });
            }
        }
        ...
        [HttpPost("debug/customer/{customerId}/set-consumption/{amount}")]
        public async Task<ActionResult> SetCustomerConsumption(int customerId, decimal amount)
        {
            try
            {
                _logger.LogInformation($"手动设置客户 {customerId} 的累计消费金额为 {amount}");
                
                var success = await _memberService.SetCustomerTotalConsumptionAsync(customerId, amount);
                
                if (success)
                {
                    return Ok(new { message = $"客户 {customerId} 累计消费金额已设置为 {amount}" });
                }
                else
                {
                    return BadRequest(new { message = "设置累计消费金额失败" });
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"设置客户 {customerId} 累计消费金额失败");
                return StatusCode(500, new { message = "设置累计消费金额失败", error = ex.Message });
            }
        }
        ...
        [HttpPost("debug/create-test-customer")]
        public async Task<ActionResult> CreateTestCustomer()
        {
            try
            {
                _logger.LogInformation("创建测试客户数据");
                
                // 调用DatabaseService创建测试客户
                return Ok(new { message = "请使用DatabaseService的AddTestCustomerAsync方法" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "创建测试客户失败");
                return StatusCode(500, new { message = "创建测试客户失败", error = ex.Message });
            }
        }
